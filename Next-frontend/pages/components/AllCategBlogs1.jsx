import Link from 'next/link'
import React from 'react'

const AllCategBlogs1 = ({category,blog}) => {
	console.log(blog)
  return (
    <>

<section className="bg-gray-100 text-gray-800">
	<div className="container max-w-6xl p-6 mx-auto space-y-6 sm:space-y-12">
		
		<div className="grid justify-center grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">

			

{blog.map((item, index) => {
        return (
          <div key={index}> 
           <Link rel="noopener noreferrer" href={`/${category}_slug/${item.slug.current}`}className="max-w-sm mx-auto group hover:no-underline focus:no-underline bg-gray-50">
				<img role="presentation" className="object-cover w-full rounded h-44 bg-gray-500" src="https://source.unsplash.com/random/480x360?1" />
				<div className="p-6 space-y-2">
					<h3 className="text-2xl font-semibold group-hover:underline group-focus:underline">{item?.title}</h3>
					<span className="text-xs text-gray-600">January 21, 2021</span>
					<p>{item.metadisc}</p>
				</div>
			</Link> 
          </div>
      );
      })} 
		
			
		</div>
		
	</div>
</section>
      
    </>
  )
}

export default AllCategBlogs1
